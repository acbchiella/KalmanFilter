...
namespace KalmanFilter {
    ...
    static void UT(
        ...
        X_sp.col(0) = x_aug + S.col(0);
        X_sp.col(n) = x_aug -S.col(0);
        ...
        for (int i = 1; i < n; i++)
        {
            X_sp.col(i) = x_aug + S.col(i);
            X_sp.col(n+i) = x_aug - S.col(i);
        ...
        error_z = Z_sp.col(0) - z;

        // Computes the first error_x.
        Eigen::MatrixXd error_x;
        error_x = X_sp.block(0,0, x.rows(), 1) - x;
        ...
        for (int i = 1; i < ns; i++)
        { 
            error_z = Z_sp.col(i) - z;
            error_x = X_sp.block(0,i, x.rows(), 1) - x;
    ...
    // The Rauch-Tung-Striebel Kalman smoother. 
    static void URTS(
        fun_T f_fun, 
        const Eigen::VectorXd &u_m, 
        const double &dt, 
